...
const rapidcsv::Document T_CSV("../datasets/job/21a/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document MI_CSV("../datasets/job/21a/mi.csv", NO_HEADERS, SEPARATOR);
auto mi = tuple(/* movie_id */ MI_CSV.GetColumn<int>(1), /* size */ static_cast<int>(MI_CSV.GetRowCount()));
const rapidcsv::Document MC_CSV("../datasets/job/21a/mc.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document LT_CSV("../datasets/job/21a/lt.csv", NO_HEADERS, SEPARATOR);
auto lt = tuple(/* id */ LT_CSV.GetColumn<int>(0), /* link */ LT_CSV.GetColumn<string>(1), /* size */ static_cast<int>(LT_CSV.GetRowCount()));
const rapidcsv::Document K_CSV("../datasets/job/21a/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document CT_CSV("../datasets/job/21a/ct.csv", NO_HEADERS, SEPARATOR);
auto ct = tuple(/* id */ CT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CT_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/21a/cn.csv", NO_HEADERS, SEPARATOR);
