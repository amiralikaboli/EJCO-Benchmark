...
const rapidcsv::Document T_CSV("../datasets/job/12b/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT2_CSV("../datasets/job/12b/it2.csv", NO_HEADERS, SEPARATOR);
auto it2 = tuple(/* id */ IT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT2_CSV.GetRowCount()));
const rapidcsv::Document IT1_CSV("../datasets/job/12b/it1.csv", NO_HEADERS, SEPARATOR);
auto it1 = tuple(/* id */ IT1_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT1_CSV.GetRowCount()));
const rapidcsv::Document CT_CSV("../datasets/job/12b/ct.csv", NO_HEADERS, SEPARATOR);
auto ct = tuple(/* id */ CT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CT_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/12b/cn.csv", NO_HEADERS, SEPARATOR);
...
interm0_trie0[get<0>(t)[t_off]][tuple<string, int>(get<1>(t)[t_off], get<1>(mi_idx)[mi_idx_off])] += 1;
}
}
}
