...
const rapidcsv::Document T_CSV("../datasets/job/16a/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document K_CSV("../datasets/job/16a/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/16a/cn.csv", NO_HEADERS, SEPARATOR);
...
auto interm0_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string, int>, 4>>(min(1000000, get<2>(mk)));
...
interm0_trie0[get<0>(mk)[mk_off]][tuple<int, string, int>(get<1>(mk)[mk_off], get<1>(t)[t_off], get<1>(mc)[mc_off])] += 1;
}
}
}
}
