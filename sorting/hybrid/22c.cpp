...
const rapidcsv::Document T_CSV("../datasets/job/22c/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document MI_IDX_CSV("../datasets/job/22c/mi_idx.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document KT_CSV("../datasets/job/22c/kt.csv", NO_HEADERS, SEPARATOR);
auto kt = tuple(/* id */ KT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(KT_CSV.GetRowCount()));
const rapidcsv::Document IT2_CSV("../datasets/job/22c/it2.csv", NO_HEADERS, SEPARATOR);
auto it2 = tuple(/* id */ IT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT2_CSV.GetRowCount()));
const rapidcsv::Document MC_CSV("../datasets/job/22c/mc.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CN_CSV("../datasets/job/22c/cn.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document K_CSV("../datasets/job/22c/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document MI_CSV("../datasets/job/22c/mi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT1_CSV("../datasets/job/22c/it1.csv", NO_HEADERS, SEPARATOR);
...
auto interm0_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string, int, string>, 4>>(min(1000000, get<3>(t)));
...
interm0_trie0[get<0>(t)[t_off]][tuple<int, string, int, string>(get<2>(t)[t_off], get<1>(t)[t_off], get<1>(mi_idx)[mi_idx_off], get<2>(mi_idx)[mi_idx_off])] += 1;
}
}
}
}
...
auto interm1_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, int, string, string, string>, 4>>(min(1000000, get<3>(mc)));
...
interm1_trie0[get<0>(mc)[mc_off]][tuple<int, int, string, string, string>(get<1>(mc)[mc_off], get<2>(mc)[mc_off], get<1>(cn)[cn_off], get<1>(interm0_tuple), get<3>(interm0_tuple))] += 1;
}
}
}
}
...
auto interm2_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string, string, string>, 4>>(min(1000000, get<2>(mk)));
...
interm2_trie0[get<0>(mk)[mk_off]][tuple<int, string, string, string>(get<1>(mk)[mk_off], get<2>(interm1_tuple), get<3>(interm1_tuple), get<4>(interm1_tuple))] += 1;
}
}
}
}
...
min_inplace(mn_interm2, {get<1>(interm2_tuple), get<2>(interm2_tuple), get<3>(interm2_tuple)});
}
