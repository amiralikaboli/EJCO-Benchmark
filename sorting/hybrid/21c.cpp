...
const rapidcsv::Document T_CSV("../datasets/job/21c/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document MI_CSV("../datasets/job/21c/mi.csv", NO_HEADERS, SEPARATOR);
auto mi = tuple(/* movie_id */ MI_CSV.GetColumn<int>(1), /* size */ static_cast<int>(MI_CSV.GetRowCount()));
const rapidcsv::Document MC_CSV("../datasets/job/21c/mc.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document LT_CSV("../datasets/job/21c/lt.csv", NO_HEADERS, SEPARATOR);
auto lt = tuple(/* id */ LT_CSV.GetColumn<int>(0), /* link */ LT_CSV.GetColumn<string>(1), /* size */ static_cast<int>(LT_CSV.GetRowCount()));
const rapidcsv::Document K_CSV("../datasets/job/21c/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document CT_CSV("../datasets/job/21c/ct.csv", NO_HEADERS, SEPARATOR);
auto ct = tuple(/* id */ CT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CT_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/21c/cn.csv", NO_HEADERS, SEPARATOR);
...
auto interm1_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string, int>, 4>>(min(1000000, get<2>(mk)));
...
interm1_trie0[get<0>(mk)[mk_off]][tuple<int, string, int>(get<1>(mk)[mk_off], get<0>(interm0_tuple), get<1>(interm0_tuple))] += 1;
}
}
}
...
auto mn_interm1 = tuple<string>(get<1>(interm1_tuple));
