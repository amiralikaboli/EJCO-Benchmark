...
const rapidcsv::Document MC_CSV("../datasets/job/8b/mc.csv", NO_HEADERS, SEPARATOR);
auto mc = tuple(/* movie_id */ MC_CSV.GetColumn<int>(1), /* company_id */ MC_CSV.GetColumn<int>(2), /* size */ static_cast<int>(MC_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/8b/cn.csv", NO_HEADERS, SEPARATOR);
auto cn = tuple(/* id */ CN_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CN_CSV.GetRowCount()));
const rapidcsv::Document CI_CSV("../datasets/job/8b/ci.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document N_CSV("../datasets/job/8b/n.csv", NO_HEADERS, SEPARATOR);
auto n = tuple(/* id */ N_CSV.GetColumn<int>(0), /* size */ static_cast<int>(N_CSV.GetRowCount()));
const rapidcsv::Document RT_CSV("../datasets/job/8b/rt.csv", NO_HEADERS, SEPARATOR);
auto rt = tuple(/* id */ RT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(RT_CSV.GetRowCount()));
const rapidcsv::Document T_CSV("../datasets/job/8b/t.csv", NO_HEADERS, SEPARATOR);
...
const auto an_it = an_trie0.find(x2);
...
min_inplace(res, {get<0>(mn_t), get<0>(mn_an)});
}
}
}
}
