...
const rapidcsv::Document T_CSV("../datasets/job/25b/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document MI_CSV("../datasets/job/25b/mi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document K_CSV("../datasets/job/25b/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document IT2_CSV("../datasets/job/25b/it2.csv", NO_HEADERS, SEPARATOR);
auto it2 = tuple(/* id */ IT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT2_CSV.GetRowCount()));
const rapidcsv::Document IT1_CSV("../datasets/job/25b/it1.csv", NO_HEADERS, SEPARATOR);
auto it1 = tuple(/* id */ IT1_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT1_CSV.GetRowCount()));
const rapidcsv::Document CI_CSV("../datasets/job/25b/ci.csv", NO_HEADERS, SEPARATOR);
auto ci = tuple(/* person_id */ CI_CSV.GetColumn<int>(1), /* movie_id */ CI_CSV.GetColumn<int>(2), /* size */ static_cast<int>(CI_CSV.GetRowCount()));
const rapidcsv::Document N_CSV("../datasets/job/25b/n.csv", NO_HEADERS, SEPARATOR);
...
auto interm1_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string, int, int, string, string>, 4>>(min(1000000, get<3>(mi)));
...
interm1_trie0[get<0>(mi)[mi_off]][tuple<int, string, int, int, string, string>(get<1>(mi)[mi_off], get<2>(mi)[mi_off], get<0>(interm0_tuple), get<2>(interm0_tuple), get<1>(interm0_tuple), get<3>(interm0_tuple))] += 1;
}
}
}
}
