...
const rapidcsv::Document MI_CSV("../datasets/job/19b/mi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CI_CSV("../datasets/job/19b/ci.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document RT_CSV("../datasets/job/19b/rt.csv", NO_HEADERS, SEPARATOR);
...
auto interm0_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string, int>, 4>>(min(1000000, get<2>(mi)));
...
interm0_trie0[get<0>(mi)[mi_off]][tuple<int, string, int>(get<1>(mi)[mi_off], get<1>(t)[t_off], get<1>(mc)[mc_off])] += 1;
}
}
}
}
...
auto interm1_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<string>, 4>>(min(1000000, get<2>(n)));
...
interm1_trie0[get<0>(n)[n_off]][tuple<string>(get<1>(n)[n_off])] += 1;
}
}
...
min_inplace(mn_interm0, {get<1>(interm0_tuple)});
}
