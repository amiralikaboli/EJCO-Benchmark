...
const rapidcsv::Document N_CSV("../datasets/job/7c/n.csv", NO_HEADERS, SEPARATOR);
auto n = tuple(/* id */ N_CSV.GetColumn<int>(0), /* name */ N_CSV.GetColumn<string>(1), /* size */ static_cast<int>(N_CSV.GetRowCount()));
const rapidcsv::Document PI_CSV("../datasets/job/7c/pi.csv", NO_HEADERS, SEPARATOR);
auto pi = tuple(/* person_id */ PI_CSV.GetColumn<int>(1), /* info_type_id */ PI_CSV.GetColumn<int>(2), /* info */ PI_CSV.GetColumn<string>(3), /* size */ static_cast<int>(PI_CSV.GetRowCount()));
const rapidcsv::Document IT_CSV("../datasets/job/7c/it.csv", NO_HEADERS, SEPARATOR);
auto it = tuple(/* id */ IT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT_CSV.GetRowCount()));
const rapidcsv::Document T_CSV("../datasets/job/7c/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document LT_CSV("../datasets/job/7c/lt.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document AN_CSV("../datasets/job/7c/an.csv", NO_HEADERS, SEPARATOR);
auto an = tuple(/* person_id */ AN_CSV.GetColumn<int>(1), /* size */ static_cast<int>(AN_CSV.GetRowCount()));

int main() {
vector<int> pi_offsets(get<3>(pi));
...
timer.Reset();
SortedDict<int, Range> pi_trie0(get<3>(pi));
for (int i = 0; i < get<3>(pi); ++i)
...
auto interm0_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<string, int, string>, 4>>(min(1000000, get<2>(n)));
...
interm0_trie0[get<0>(n)[n_off]][tuple<string, int, string>(get<1>(n)[n_off], get<1>(pi)[pi_off], get<2>(pi)[pi_off])] += 1;
}
}
}
}
...
auto interm1_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int>, 4>>(min(1000000, get<1>(t)));
for (auto t_off = 0; t_off < get<1>(t); ++t_off) {
...
interm1_trie0[get<0>(t)[t_off]][tuple<int>(get<1>(ml)[ml_off])] += 1;
}
}
}
}
...
min_inplace(mn_interm0, {get<0>(interm0_tuple), get<2>(interm0_tuple)});
}
