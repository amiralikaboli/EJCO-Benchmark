...
const rapidcsv::Document T_CSV("../datasets/job/13c/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document KT_CSV("../datasets/job/13c/kt.csv", NO_HEADERS, SEPARATOR);
auto kt = tuple(/* id */ KT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(KT_CSV.GetRowCount()));
const rapidcsv::Document IT2_CSV("../datasets/job/13c/it2.csv", NO_HEADERS, SEPARATOR);
auto it2 = tuple(/* id */ IT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT2_CSV.GetRowCount()));
const rapidcsv::Document IT_CSV("../datasets/job/13c/it.csv", NO_HEADERS, SEPARATOR);
auto it = tuple(/* id */ IT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT_CSV.GetRowCount()));
const rapidcsv::Document CT_CSV("../datasets/job/13c/ct.csv", NO_HEADERS, SEPARATOR);
auto ct = tuple(/* id */ CT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CT_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/13c/cn.csv", NO_HEADERS, SEPARATOR);
...
auto interm0_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<string, int, string, int>, 4>>(min(1000000, get<3>(t)));
...
interm0_trie0[get<0>(t)[t_off]][tuple<string, int, string, int>(get<1>(t)[t_off], get<2>(t)[t_off], get<2>(miidx)[miidx_off], get<1>(miidx)[miidx_off])] += 1;
}
}
}
...
auto mn_interm0 = tuple<string, string>(get<0>(interm0_tuple), get<2>(interm0_tuple));
