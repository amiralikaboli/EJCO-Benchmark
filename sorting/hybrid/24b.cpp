...
auto interm1_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, int, string, int>, 4>>(min(1000000, get<2>(mi)));
...
interm1_trie0[get<0>(mi)[mi_off]][tuple<int, int, string, int>(get<1>(mi)[mi_off], get<0>(interm0_tuple), get<1>(interm0_tuple), get<1>(mc)[mc_off])] += 1;
}
}
}
}
...
auto interm2_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<string>, 4>>(min(1000000, get<2>(n)));
...
interm2_trie0[get<0>(n)[n_off]][tuple<string>(get<1>(n)[n_off])] += 1;
}
}
...
min_inplace(mn_interm2, {get<0>(interm2_tuple)});
}
