...
const rapidcsv::Document T_CSV("../datasets/job/11d/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document MC_CSV("../datasets/job/11d/mc.csv", NO_HEADERS, SEPARATOR);
auto mc = tuple(/* movie_id */ MC_CSV.GetColumn<int>(1), /* company_id */ MC_CSV.GetColumn<int>(2), /* company_type_id */ MC_CSV.GetColumn<int>(3), /* note */ MC_CSV.GetColumn<string>(4), /* size */ static_cast<int>(MC_CSV.GetRowCount()));
...
const rapidcsv::Document K_CSV("../datasets/job/11d/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document CT_CSV("../datasets/job/11d/ct.csv", NO_HEADERS, SEPARATOR);
auto ct = tuple(/* id */ CT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CT_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/11d/cn.csv", NO_HEADERS, SEPARATOR);
...
vector<int> mc_offsets(get<4>(mc));
...
SortedDict<int, Range> mc_trie0(get<4>(mc));
for (int i = 0; i < get<4>(mc); ++i)
...
auto mn_mc = tuple<string>(get<3>(mc)[mc_off]);
...
min_inplace(res, {get<0>(mn_interm0), get<0>(mn_mc), get<0>(mn_cn)});
}
}
}
}
