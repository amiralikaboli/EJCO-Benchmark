...
const rapidcsv::Document T_CSV("../datasets/job/14c/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document MI_IDX_CSV("../datasets/job/14c/mi_idx.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document MI_CSV("../datasets/job/14c/mi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document KT_CSV("../datasets/job/14c/kt.csv", NO_HEADERS, SEPARATOR);
auto kt = tuple(/* id */ KT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(KT_CSV.GetRowCount()));
const rapidcsv::Document K_CSV("../datasets/job/14c/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document IT2_CSV("../datasets/job/14c/it2.csv", NO_HEADERS, SEPARATOR);
auto it2 = tuple(/* id */ IT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT2_CSV.GetRowCount()));
const rapidcsv::Document IT1_CSV("../datasets/job/14c/it1.csv", NO_HEADERS, SEPARATOR);
...
auto interm0_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string, int, int, string>, 4>>(min(1000000, get<2>(mk)));
...
interm0_trie0[get<0>(mk)[mk_off]][tuple<int, string, int, int, string>(get<1>(mk)[mk_off], get<1>(t)[t_off], get<2>(t)[t_off], get<1>(mi_idx)[mi_idx_off], get<2>(mi_idx)[mi_idx_off])] += 1;
}
}
}
}
...
const auto x2 = get<0>(interm0_tuple);
const auto k_it = k_trie0.find(x2);
if (k_it != k_trie0.end()) {
const auto &k_trie1 = k_it->second;
const auto x3 = get<3>(interm0_tuple);
...
auto mn_interm0 = tuple<string, string>(get<1>(interm0_tuple), get<4>(interm0_tuple));
