...
const rapidcsv::Document T_CSV("../datasets/job/31b/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document MI_CSV("../datasets/job/31b/mi.csv", NO_HEADERS, SEPARATOR);
...
auto interm0_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string, int, string>, 4>>(min(1000000, get<2>(mk)));
...
interm0_trie0[get<0>(mk)[mk_off]][tuple<int, string, int, string>(get<1>(mk)[mk_off], get<1>(t)[t_off], get<1>(mi_idx)[mi_idx_off], get<2>(mi_idx)[mi_idx_off])] += 1;
}
}
}
}
...
auto interm1_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string, int, int, string, string, int>, 4>>(min(1000000, get<3>(mi)));
...
const auto x2 = get<2>(interm0_tuple);
...
interm1_trie0[get<0>(mi)[mi_off]][tuple<int, string, int, int, string, string, int>(get<1>(mi)[mi_off], get<2>(mi)[mi_off], get<0>(interm0_tuple), get<2>(interm0_tuple), get<1>(interm0_tuple), get<3>(interm0_tuple), get<1>(mc)[mc_off])] += 1;
}
}
}
}
...
min_inplace(mn_interm1, {get<1>(interm1_tuple), get<4>(interm1_tuple), get<5>(interm1_tuple)});
}
