...
const rapidcsv::Document CCT1_CSV("../datasets/job/29c/cct1.csv", NO_HEADERS, SEPARATOR);
auto cct1 = tuple(/* id */ CCT1_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CCT1_CSV.GetRowCount()));
const rapidcsv::Document CCT2_CSV("../datasets/job/29c/cct2.csv", NO_HEADERS, SEPARATOR);
auto cct2 = tuple(/* id */ CCT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CCT2_CSV.GetRowCount()));
const rapidcsv::Document T_CSV("../datasets/job/29c/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CN_CSV("../datasets/job/29c/cn.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document K_CSV("../datasets/job/29c/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document MI_CSV("../datasets/job/29c/mi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT_CSV("../datasets/job/29c/it.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT3_CSV("../datasets/job/29c/it3.csv", NO_HEADERS, SEPARATOR);
auto it3 = tuple(/* id */ IT3_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT3_CSV.GetRowCount()));
const rapidcsv::Document N_CSV("../datasets/job/29c/n.csv", NO_HEADERS, SEPARATOR);
auto n = tuple(/* id */ N_CSV.GetColumn<int>(0), /* name */ N_CSV.GetColumn<string>(1), /* size */ static_cast<int>(N_CSV.GetRowCount()));
const rapidcsv::Document CI_CSV("../datasets/job/29c/ci.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document RT_CSV("../datasets/job/29c/rt.csv", NO_HEADERS, SEPARATOR);
...
auto interm1_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<string>, 4>>(min(1000000, get<2>(t)));
...
interm1_trie0[get<0>(t)[t_off]][tuple<string>(get<1>(t)[t_off])] += 1;
}
}
SortedDict<int, int> cn_trie0(get<1>(cn));
for (int i = 0; i < get<1>(cn); ++i)
cn_trie0[get<0>(cn)[cn_offsets[i]]] += 1;
auto interm2_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string>, 4>>(min(1000000, get<2>(mc)));
...
interm2_trie0[get<0>(mc)[mc_off]][tuple<int, string>(get<1>(mc)[mc_off], get<0>(interm1_tuple))] += 1;
}
}
}
}
...
auto interm3_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string>, 4>>(min(1000000, get<2>(mk)));
...
interm3_trie0[get<0>(mk)[mk_off]][tuple<int, string>(get<1>(mk)[mk_off], get<1>(interm2_tuple))] += 1;
}
}
}
}
...
auto interm4_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int, string>, 4>>(min(1000000, get<2>(mi)));
...
interm4_trie0[get<0>(mi)[mi_off]][tuple<int, string>(get<1>(mi)[mi_off], get<1>(interm3_tuple))] += 1;
}
}
}
}
...
auto interm5_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<int>, 4>>(min(1000000, get<2>(pi)));
...
interm5_trie0[get<0>(pi)[pi_off]][tuple<int>(get<1>(pi)[pi_off])] += 1;
}
}
}
auto interm6_trie0 = phmap::flat_hash_map<int, smallvecdict<tuple<string>, 4>>(min(1000000, get<2>(n)));
...
interm6_trie0[get<0>(n)[n_off]][tuple<string>(get<1>(n)[n_off])] += 1;
}
}
...
const auto interm6_it = interm6_trie0.find(x2);
...
for (const auto &interm6_tuple: interm6_trie1) {
min_inplace(mn_interm6, {get<0>(interm6_tuple)});
}
auto mn_interm4 = tuple<string>(STRING_MAX);
for (const auto &interm4_tuple: interm4_trie1) {
min_inplace(mn_interm4, {get<1>(interm4_tuple)});
}
