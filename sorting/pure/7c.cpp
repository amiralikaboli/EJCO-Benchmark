...
interm0.emplace_back(get<0>(n)[n_off], get<1>(n)[n_off], get<1>(pi)[pi_off], get<2>(pi)[pi_off]);}
}
}
}
...
interm1.emplace_back(get<0>(t)[t_off], get<1>(ml)[ml_off]);}
}
}
}
sort(interm1.begin(), interm1.end(), [](const tuple<int, int> &a, const tuple<int, int> &b) { return get<0>(a) < get<0>(b); });
...
min_inplace(mn_interm0, {get<1>(interm0_tuple), get<3>(interm0_tuple)});
}
