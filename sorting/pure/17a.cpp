...
const rapidcsv::Document K_CSV("../datasets/job/17a/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/17a/cn.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document N_CSV("../datasets/job/17a/n.csv", NO_HEADERS, SEPARATOR);
...
interm0.emplace_back(get<0>(mk)[mk_off], get<1>(mk)[mk_off], get<1>(mc)[mc_off]);}
}
}
}
}
