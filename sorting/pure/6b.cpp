...
const rapidcsv::Document T_CSV("../datasets/job/6b/t.csv", NO_HEADERS, SEPARATOR);
auto t = tuple(/* id */ T_CSV.GetColumn<int>(0), /* title */ T_CSV.GetColumn<string>(1), /* size */ static_cast<int>(T_CSV.GetRowCount()));
const rapidcsv::Document K_CSV("../datasets/job/6b/k.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document N_CSV("../datasets/job/6b/n.csv", NO_HEADERS, SEPARATOR);
...
vector<tuple<int, int, string, string>> interm0;
...
interm0.emplace_back(get<0>(mk)[mk_off], get<1>(mk)[mk_off], get<1>(t)[t_off], get<1>(k)[k_off]);}
}
}
}
}
...
sort(interm0.begin(), interm0.end(), [](const tuple<int, int, string, string> &a, const tuple<int, int, string, string> &b) { return get<0>(a) < get<0>(b); });
...
min_inplace(mn_interm0, {get<2>(interm0_tuple), get<3>(interm0_tuple)});
}
