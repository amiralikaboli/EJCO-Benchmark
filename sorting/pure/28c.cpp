...
const rapidcsv::Document CCT1_CSV("../datasets/job/28c/cct1.csv", NO_HEADERS, SEPARATOR);
auto cct1 = tuple(/* id */ CCT1_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CCT1_CSV.GetRowCount()));
const rapidcsv::Document CCT2_CSV("../datasets/job/28c/cct2.csv", NO_HEADERS, SEPARATOR);
auto cct2 = tuple(/* id */ CCT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CCT2_CSV.GetRowCount()));
const rapidcsv::Document MI_IDX_CSV("../datasets/job/28c/mi_idx.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT2_CSV("../datasets/job/28c/it2.csv", NO_HEADERS, SEPARATOR);
auto it2 = tuple(/* id */ IT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT2_CSV.GetRowCount()));
const rapidcsv::Document T_CSV("../datasets/job/28c/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document KT_CSV("../datasets/job/28c/kt.csv", NO_HEADERS, SEPARATOR);
auto kt = tuple(/* id */ KT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(KT_CSV.GetRowCount()));
const rapidcsv::Document MC_CSV("../datasets/job/28c/mc.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CN_CSV("../datasets/job/28c/cn.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document K_CSV("../datasets/job/28c/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document MI_CSV("../datasets/job/28c/mi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT1_CSV("../datasets/job/28c/it1.csv", NO_HEADERS, SEPARATOR);
...
interm1.emplace_back(get<0>(mi_idx)[mi_idx_off], get<1>(mi_idx)[mi_idx_off], get<0>(mi_idx)[mi_idx_off], get<2>(mi_idx)[mi_idx_off]);}
}
}
...
vector<tuple<int, int, int, string, string>> interm2;
...
interm2.emplace_back(get<0>(t)[t_off], get<2>(t)[t_off], get<0>(t)[t_off], get<1>(t)[t_off], get<3>(interm1_tuple));}
}
}
}
...
sort(interm2.begin(), interm2.end(), [](const tuple<int, int, int, string, string> &a, const tuple<int, int, int, string, string> &b) { return get<0>(a) < get<0>(b); });
...
vector<tuple<int, int, int, int, string, string, string>> interm3;
...
interm3.emplace_back(get<0>(mc)[mc_off], get<1>(mc)[mc_off], get<2>(mc)[mc_off], get<0>(mc)[mc_off], get<1>(cn)[cn_off], get<3>(interm2_tuple), get<4>(interm2_tuple));}
}
}
}
}
...
sort(interm3.begin(), interm3.end(), [](const tuple<int, int, int, int, string, string, string> &a, const tuple<int, int, int, int, string, string, string> &b) { return get<0>(a) < get<0>(b); });
auto interm3_trie0 = SortedDict<int, Range>(interm3.size());
for (int i = 0; i < interm3.size(); ++i)
interm3_trie0[get<0>(interm3[i])][i] += 1;
vector<tuple<int, int, int, string, string, string>> interm4;
...
interm4.emplace_back(get<0>(mk)[mk_off], get<1>(mk)[mk_off], get<0>(mk)[mk_off], get<4>(interm3_tuple), get<5>(interm3_tuple), get<6>(interm3_tuple));}
}
}
}
...
sort(interm4.begin(), interm4.end(), [](const tuple<int, int, int, string, string, string> &a, const tuple<int, int, int, string, string, string> &b) { return get<0>(a) < get<0>(b); });
...
const auto interm4_it = interm4_trie0.find(x1);
...
min_inplace(mn_interm4, {get<3>(interm4_tuple), get<4>(interm4_tuple), get<5>(interm4_tuple)});
}
