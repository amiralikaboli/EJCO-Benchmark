...
const rapidcsv::Document T_CSV("../datasets/job/10c/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CN_CSV("../datasets/job/10c/cn.csv", NO_HEADERS, SEPARATOR);
auto cn = tuple(/* id */ CN_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CN_CSV.GetRowCount()));
const rapidcsv::Document CI_CSV("../datasets/job/10c/ci.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document RT_CSV("../datasets/job/role_type.csv", NO_HEADERS, SEPARATOR);
...
vector<tuple<int, int, int, string>> interm0;
...
interm0.emplace_back(get<0>(mc)[mc_off], get<2>(mc)[mc_off], get<1>(mc)[mc_off], get<1>(t)[t_off]);}
}
}
}
}
...
sort(interm0.begin(), interm0.end(), [](const tuple<int, int, int, string> &a, const tuple<int, int, int, string> &b) { return get<0>(a) < get<0>(b); });
...
min_inplace(mn_interm0, {get<3>(interm0_tuple)});
}
