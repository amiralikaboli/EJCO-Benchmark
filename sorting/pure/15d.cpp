...
const rapidcsv::Document T_CSV("../datasets/job/15d/t.csv", NO_HEADERS, SEPARATOR);
auto t = tuple(/* id */ T_CSV.GetColumn<int>(0), /* title */ T_CSV.GetColumn<string>(1), /* size */ static_cast<int>(T_CSV.GetRowCount()));
const rapidcsv::Document MI_CSV("../datasets/job/15d/mi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT1_CSV("../datasets/job/15d/it1.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CN_CSV("../datasets/job/15d/cn.csv", NO_HEADERS, SEPARATOR);
...
auto at = tuple(/* movie_id */ AT_CSV.GetColumn<int>(1), /* title */ AT_CSV.GetColumn<string>(2), /* size */ static_cast<int>(AT_CSV.GetRowCount()));
...
vector<int> at_offsets(get<2>(at));
...
SortedDict<int, Range> at_trie0(get<2>(at));
for (int i = 0; i < get<2>(at); ++i)
at_trie0[get<0>(at)[at_offsets[i]]][i] += 1;
...
auto mn_at = tuple<string>(STRING_MAX);
for (auto at_i = at_trie1.left(); at_i < at_trie1.right(); ++at_i) {
const auto at_off = at_offsets[at_i];
min_inplace(mn_at, {get<1>(at)[at_off]});
}
min_inplace(res, {get<0>(mn_interm0), get<0>(mn_at)});
}
}
}
}
