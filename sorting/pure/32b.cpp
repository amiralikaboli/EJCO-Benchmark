...
vector<tuple<int, string, int, int, string>> interm0;
...
interm0.emplace_back(get<0>(t1)[t1_off], get<1>(t1)[t1_off], get<1>(ml)[ml_off], get<2>(ml)[ml_off], get<1>(t2)[t2_off]);}
}
}
}
}
sort(interm0.begin(), interm0.end(), [](const tuple<int, string, int, int, string> &a, const tuple<int, string, int, int, string> &b) { return get<0>(a) < get<0>(b); });
