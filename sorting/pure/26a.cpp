...
const rapidcsv::Document CCT1_CSV("../datasets/job/26a/cct1.csv", NO_HEADERS, SEPARATOR);
auto cct1 = tuple(/* id */ CCT1_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CCT1_CSV.GetRowCount()));
const rapidcsv::Document CCT2_CSV("../datasets/job/26a/cct2.csv", NO_HEADERS, SEPARATOR);
auto cct2 = tuple(/* id */ CCT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CCT2_CSV.GetRowCount()));
const rapidcsv::Document MI_IDX_CSV("../datasets/job/26a/mi_idx.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT2_CSV("../datasets/job/26a/it2.csv", NO_HEADERS, SEPARATOR);
auto it2 = tuple(/* id */ IT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT2_CSV.GetRowCount()));
const rapidcsv::Document T_CSV("../datasets/job/26a/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document KT_CSV("../datasets/job/26a/kt.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document K_CSV("../datasets/job/26a/k.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CHN_CSV("../datasets/job/26a/chn.csv", NO_HEADERS, SEPARATOR);
...
interm3.emplace_back(get<0>(mk)[mk_off], get<1>(mk)[mk_off], get<0>(mk)[mk_off], get<3>(interm2_tuple), get<4>(interm2_tuple));}
}
}
}
...
min_inplace(res, {get<0>(mn_n), get<0>(mn_chn), get<0>(mn_interm3), get<1>(mn_interm3)});
}
}
}
}
