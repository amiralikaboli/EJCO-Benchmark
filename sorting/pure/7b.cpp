...
const rapidcsv::Document N_CSV("../datasets/job/7b/n.csv", NO_HEADERS, SEPARATOR);
auto n = tuple(/* id */ N_CSV.GetColumn<int>(0), /* name */ N_CSV.GetColumn<string>(1), /* size */ static_cast<int>(N_CSV.GetRowCount()));
const rapidcsv::Document PI_CSV("../datasets/job/7b/pi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT_CSV("../datasets/job/7b/it.csv", NO_HEADERS, SEPARATOR);
auto it = tuple(/* id */ IT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT_CSV.GetRowCount()));
const rapidcsv::Document T_CSV("../datasets/job/7b/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document LT_CSV("../datasets/job/7b/lt.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document AN_CSV("../datasets/job/7b/an.csv", NO_HEADERS, SEPARATOR);
...
interm0.emplace_back(get<0>(n)[n_off], get<1>(n)[n_off], get<1>(pi)[pi_off]);}
}
}
}
...
vector<tuple<int, string, int>> interm1;
...
interm1.emplace_back(get<0>(t)[t_off], get<1>(t)[t_off], get<1>(ml)[ml_off]);}
}
}
}
...
sort(interm1.begin(), interm1.end(), [](const tuple<int, string, int> &a, const tuple<int, string, int> &b) { return get<0>(a) < get<0>(b); });
