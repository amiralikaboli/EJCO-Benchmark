...
const rapidcsv::Document T_CSV("../datasets/job/1d/t.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document MC_CSV("../datasets/job/1d/mc.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT_CSV("../datasets/job/1d/it.csv", NO_HEADERS, SEPARATOR);
auto it = tuple(/* id */ IT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT_CSV.GetRowCount()));
const rapidcsv::Document CT_CSV("../datasets/job/1d/ct.csv", NO_HEADERS, SEPARATOR);
...
vector<tuple<int, string, int, int>> interm0;
...
interm0.emplace_back(get<0>(t)[t_off], get<1>(t)[t_off], get<2>(t)[t_off], get<1>(mi_idx)[mi_idx_off]);}
}
}
sort(interm0.begin(), interm0.end(), [](const tuple<int, string, int, int> &a, const tuple<int, string, int, int> &b) { return get<0>(a) < get<0>(b); });
...
auto mn_interm0 = tuple<string, int>(get<1>(interm0_tuple), get<2>(interm0_tuple));
