...
const rapidcsv::Document LT_CSV("../datasets/job/27a/lt.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CCT1_CSV("../datasets/job/27a/cct1.csv", NO_HEADERS, SEPARATOR);
auto cct1 = tuple(/* id */ CCT1_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CCT1_CSV.GetRowCount()));
const rapidcsv::Document CCT2_CSV("../datasets/job/27a/cct2.csv", NO_HEADERS, SEPARATOR);
auto cct2 = tuple(/* id */ CCT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CCT2_CSV.GetRowCount()));
const rapidcsv::Document T_CSV("../datasets/job/27a/t.csv", NO_HEADERS, SEPARATOR);
auto t = tuple(/* id */ T_CSV.GetColumn<int>(0), /* title */ T_CSV.GetColumn<string>(1), /* size */ static_cast<int>(T_CSV.GetRowCount()));
const rapidcsv::Document MC_CSV("../datasets/job/27a/mc.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CT_CSV("../datasets/job/27a/ct.csv", NO_HEADERS, SEPARATOR);
auto ct = tuple(/* id */ CT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CT_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/27a/cn.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document K_CSV("../datasets/job/27a/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document MI_CSV("../datasets/job/27a/mi.csv", NO_HEADERS, SEPARATOR);
...
interm0.emplace_back(get<0>(ml)[ml_off], get<1>(ml)[ml_off], get<1>(lt)[lt_off]);}
}
}
...
vector<tuple<int, int, int, int, string>> interm1;
...
interm1.emplace_back(get<0>(cc)[cc_off], get<1>(cc)[cc_off], get<2>(cc)[cc_off], get<0>(cc)[cc_off], get<2>(interm0_tuple));}
}
}
}
}
sort(interm1.begin(), interm1.end(), [](const tuple<int, int, int, int, string> &a, const tuple<int, int, int, int, string> &b) { return get<0>(a) < get<0>(b); });
auto interm1_trie0 = SortedDict<int, Range>(interm1.size());
for (int i = 0; i < interm1.size(); ++i)
interm1_trie0[get<0>(interm1[i])][i] += 1;
vector<tuple<int, int, string, string>> interm2;
...
interm2.emplace_back(get<0>(t)[t_off], get<0>(t)[t_off], get<1>(t)[t_off], get<4>(interm1_tuple));}
}
}
SortedDict<int, int> ct_trie0(get<1>(ct));
for (int i = 0; i < get<1>(ct); ++i)
ct_trie0[get<0>(ct)[ct_offsets[i]]] += 1;
sort(interm2.begin(), interm2.end(), [](const tuple<int, int, string, string> &a, const tuple<int, int, string, string> &b) { return get<0>(a) < get<0>(b); });
...
interm3.emplace_back(get<0>(mc)[mc_off], get<2>(mc)[mc_off], get<1>(mc)[mc_off], get<0>(mc)[mc_off], get<1>(cn)[cn_off], get<2>(interm2_tuple), get<3>(interm2_tuple));}
}
}
}
}
