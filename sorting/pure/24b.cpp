...
const rapidcsv::Document T_CSV("../datasets/job/24b/t.csv", NO_HEADERS, SEPARATOR);
auto t = tuple(/* id */ T_CSV.GetColumn<int>(0), /* title */ T_CSV.GetColumn<string>(1), /* size */ static_cast<int>(T_CSV.GetRowCount()));
const rapidcsv::Document MI_CSV("../datasets/job/24b/mi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document K_CSV("../datasets/job/24b/k.csv", NO_HEADERS, SEPARATOR);
auto k = tuple(/* id */ K_CSV.GetColumn<int>(0), /* size */ static_cast<int>(K_CSV.GetRowCount()));
const rapidcsv::Document IT_CSV("../datasets/job/24b/it.csv", NO_HEADERS, SEPARATOR);
auto it = tuple(/* id */ IT_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT_CSV.GetRowCount()));
const rapidcsv::Document CN_CSV("../datasets/job/24b/cn.csv", NO_HEADERS, SEPARATOR);
auto cn = tuple(/* id */ CN_CSV.GetColumn<int>(0), /* size */ static_cast<int>(CN_CSV.GetRowCount()));
const rapidcsv::Document N_CSV("../datasets/job/24b/n.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document CI_CSV("../datasets/job/24b/ci.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document RT_CSV("../datasets/job/24b/rt.csv", NO_HEADERS, SEPARATOR);
...
vector<tuple<int, int, int, int, string, int>> interm1;
...
const auto x3 = get<1>(mc)[mc_off];
const auto cn_it = cn_trie0.find(x3);
if (cn_it != cn_trie0.end()) {
const auto &cn_trie1 = cn_it->second;
interm1.emplace_back(get<0>(mi)[mi_off], get<0>(mi)[mi_off], get<1>(mi)[mi_off], get<1>(interm0_tuple), get<2>(interm0_tuple), get<1>(mc)[mc_off]);}
}
}
}
}
...
vector<tuple<int, string>> interm2;
...
interm2.emplace_back(get<0>(n)[n_off], get<1>(n)[n_off]);}
}
SortedDict<int, int> rt_trie0(get<1>(rt));
for (int i = 0; i < get<1>(rt); ++i)
rt_trie0[get<0>(rt)[rt_offsets[i]]] += 1;
sort(interm1.begin(), interm1.end(), [](const tuple<int, int, int, int, string, int> &a, const tuple<int, int, int, int, string, int> &b) { return get<0>(a) < get<0>(b); });
auto interm1_trie0 = SortedDict<int, Range>(interm1.size());
for (int i = 0; i < interm1.size(); ++i)
interm1_trie0[get<0>(interm1[i])][i] += 1;
sort(interm2.begin(), interm2.end(), [](const tuple<int, string> &a, const tuple<int, string> &b) { return get<0>(a) < get<0>(b); });
...
const auto interm2_it = interm2_trie0.find(x2);
...
auto mn_interm2 = tuple<string>(STRING_MAX);
...
min_inplace(mn_interm2, {get<1>(interm2_tuple)});
}
...
min_inplace(mn_interm1, {get<4>(interm1_tuple)});
}
min_inplace(res, {get<0>(mn_chn), get<0>(mn_interm2), get<0>(mn_interm1)});
}
}
}
}
