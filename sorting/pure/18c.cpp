...
const rapidcsv::Document MI_CSV("../datasets/job/18c/mi.csv", NO_HEADERS, SEPARATOR);
...
const rapidcsv::Document IT2_CSV("../datasets/job/18c/it2.csv", NO_HEADERS, SEPARATOR);
auto it2 = tuple(/* id */ IT2_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT2_CSV.GetRowCount()));
const rapidcsv::Document IT1_CSV("../datasets/job/18c/it1.csv", NO_HEADERS, SEPARATOR);
auto it1 = tuple(/* id */ IT1_CSV.GetColumn<int>(0), /* size */ static_cast<int>(IT1_CSV.GetRowCount()));
const rapidcsv::Document CI_CSV("../datasets/job/18c/ci.csv", NO_HEADERS, SEPARATOR);
auto ci = tuple(/* person_id */ CI_CSV.GetColumn<int>(1), /* movie_id */ CI_CSV.GetColumn<int>(2), /* size */ static_cast<int>(CI_CSV.GetRowCount()));
const rapidcsv::Document N_CSV("../datasets/job/18c/n.csv", NO_HEADERS, SEPARATOR);
...
vector<tuple<int, int, int, string, int, string, string>> interm1;
...
interm1.emplace_back(get<0>(mi)[mi_off], get<0>(mi)[mi_off], get<1>(mi)[mi_off], get<2>(mi)[mi_off], get<2>(interm0_tuple), get<1>(interm0_tuple), get<3>(interm0_tuple));}
}
}
}
}
sort(interm1.begin(), interm1.end(), [](const tuple<int, int, int, string, int, string, string> &a, const tuple<int, int, int, string, int, string, string> &b) { return get<0>(a) < get<0>(b); });
...
min_inplace(mn_interm1, {get<3>(interm1_tuple), get<5>(interm1_tuple), get<6>(interm1_tuple)});
}
